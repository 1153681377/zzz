		...
		success: function(data) {
			...
			for(var i in data.pu) {
				...
				var fileedit = "<button class='file-edit' tabindex='0'><i class='iconfont icon-dian'></i></button>" +
					...
					"<button class='rename' data-filename='" + data.pu[i].fullname + "'>重命名</button><button class='delect' data-filename='" + data.pu[i].fullname + "'>删除</button>" +
					"</div>";
				...
				$(".list-header").off('click', '.rename').on('click', ".rename", function(e) {
					rename($(this).data("filename"));
					e.stopPropagation();
				});
				$(".list-header").off('click', '.delect').on('click', ".delect", function(e) {
					delect($(this).data("filename"));
					e.stopPropagation();
				});
				var tdxz = "<td>" + div + "</td>";
				var td1 = "<td>" + box + "</td>";
